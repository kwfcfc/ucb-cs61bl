...
/**
 * Tests the basic case for Hyponyms where the list of words is one word long,
 * and k is non zero.
 */
public class TestMultiWordKN0Hyponyms {
    // this case doesn't use the NGrams dataset at all, so the choice of files is
    // irrelevant
    // ngrams files
    ...
    @Test
    public void testSmallK3() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                SMALL_WORDS_FILE, TOTAL_COUNTS_FILE, LARGE_SYNSET_FILE, LARGE_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.addAll(List.of("operation", "understanding"));

        NgordnetQuery nq = new NgordnetQuery(words, 1470, 2019, 3);
        String actual = studentHandler.handle(nq);
        String expected = "[find, light, policy]";
        assertThat(actual).isEqualTo(expected);
    }

    @Test
    public void testLargeK2() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                SMALL_WORDS_FILE, TOTAL_COUNTS_FILE, LARGE_SYNSET_FILE, LARGE_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.addAll(List.of("instrument", "microtome", "instrument"));
    ...
    @Test
    public void testLargeK8() {
        NgordnetQueryHandler studentHandler = AutograderBuddy.getHyponymsHandler(
                SMALL_WORDS_FILE, TOTAL_COUNTS_FILE, LARGE_SYNSET_FILE, LARGE_HYPONYM_FILE);
        List<String> words = new ArrayList<>();
        words.addAll(List.of("content", "key"));

        NgordnetQuery nq = new NgordnetQuery(words, 1470, 2019, 8);
        String actual = studentHandler.handle(nq);
        String expected = "[key, return]";
        assertThat(actual).isEqualTo(expected);
    }
}
